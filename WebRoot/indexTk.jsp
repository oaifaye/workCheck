...
<script type="text/javascript">
		...
		$("#wentikaoqin").click(function(){
			$(".table tr").each(function(){
				if($(this).attr("class")=="tableHead"||$(this).attr("class")=="trContent"){
					$(this).show();
				}else{
					$(this).hide();
				}
		...
		//旷工
		$("#kuanggong").click(function(){
			$(".table tr").each(function(){
				if($(this).attr("class")=="tableHead"||$(this).attr("class")=="trKuangGong"){
					$(this).show();
				}else{
					$(this).hide();
				}
		...
		$("#buKuang").click(function(){
			$(".trKuangGong").hide();
			$(".trContent").add(".trJiaban").add(".yincang").add(".trLeave").show();
			$(document).scrollTop(0);
		});
		
		$("#kuang").click(function(){
			$(".trKuangGong").show();
			$(".trContent").add(".trJiaban").add(".yincang").add(".trLeave").hide();
			$(document).scrollTop(0);
		});
		
		$("#qingjia").click(function(){
			$(".trLeave").show();
			$(".trContent").add(".trJiaban").add(".yincang").add(".trKuangGong").hide();
			$(document).scrollTop(0);
		});
		//双击tr选出全部这个人的信息
		$("tr").dblclick(function(){
			...
			if($workerName!=""){
				$(".trContent").add(".trJiaban").add(".trKuangGong").add(".yincang").add(".trLeave").hide();
				$("tr").each(function(){
		...
		$("#exportAbnormalTime").click(function(){
			...
			*/
			var sFeatures = "height=600, width=600, scrollbars=yes,top=100,left=500";
			window.open( "exportAbnormalTimeTk.action", '', sFeatures );
		});
	});
...
<div class="cleft_box">
	...
	<input type="button" id="wentikaoqin" value="显示迟到人员" />
	<input type="button" id="kuanggong" value="显示旷工人员" /><br />
	<input type="button" id="qingjia" value="显示请假人员" /><br />
	<input type="button" id="exportAbnormalTime" value="导出加班表格" /><br /><br />
	<input type="button" onclick="javascript:location.href='index.jsp'" style="width: 100px;height: 100px;background: blue;font-size: 25px;" value="切旧版" />
...
<!-- 正文 -->
	<div class="center">
		<div align="center">
			<s:form action="aainputTk" namespace="/" id="form1" method="post" enctype="multipart/form-data">
				<s:file id="file1" name="uploadFile" label="天科云样表"></s:file>
				<s:file id="file2" name="uploadFile" label="请假申请单"></s:file>
				<s:file id="file3" name="uploadFile" label="带薪休假申请表"></s:file>
				<s:textfield cssStyle="width:500px;" name="needDept" label="需要统计的部门"></s:textfield>
				<s:textfield cssStyle="width:500px;" name="needSite" label="需要统计的打卡地点"></s:textfield>
		...
		<table class="table">
			...
			<s:iterator value="workerList" status="st">
				...
				<s:elseif test="flog==4">
					<tr bgcolor="#00FF00" class="trLeave">
				</s:elseif>
				<s:else>
					<tr class="yincang">
				</s:else>
				...
				<td><s:property value="signTime1" /></td>
				<td title="<s:property value="signSite1" />" ><div style="height:20px; width: 100px;overflow: hidden;"><s:property value="signSite1" /></div></td>
				<td><s:property value="signTime2" /></td>
				<td title="<s:property value="signSite2" />"><div style="height:20px; width: 100px;overflow: hidden;"><s:property value="signSite2" /></div></td>
				<td><s:property value="week" /></td>
				<td id="abnormalTime" ><s:property value="abnormalTime" /></td>
				<td><s:property value="flogName" /></td>
